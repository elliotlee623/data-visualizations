...
$(function() {
  $.getJSON("res/paths.json")
   .done(function(data) {
     var graph = data["graph"];
     var paths = data["paths"];
     renderStory("#sad", graph, paths[0].path);
     renderStory("#happy", graph, paths[paths.length - 1].path);
   })
...
var renderStory = function(domId, graph, path) {
  var nodes = graph.nodes;
  var edges = graph.links;

  var nodes_by_id = _.groupBy(nodes, "id");
  var edges_by_id = _.groupBy(edges, function(e) { return nodes[ e["source"] ]["id"]; });

  _.forEach(edges_by_id, function (value, key) {
    edges_by_id[key] = _.groupBy(value, function(e) { return nodes[ e["target"] ]["id"]; });
  })
  ...
  for (var i = 0; i < path.length - 1; i++) {
    ...
    html += '<div class="narrative">' + nodes_by_id[p_cur][0].narrative + '</div>';
    html += '<div class="prev">' + edges_by_id[p_cur][p_next][0].text + '</div>';
  }
  html += '<div class="narrative">' + nodes_by_id[path[path.length - 1]][0].narrative + '</div>';
...
var renderStoryX = function(node_index, e) {
  var html = "";
  if (e != null) {
